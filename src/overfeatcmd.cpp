...
int main(int argc, char* argv[]) {
  // read arguments
  if (argc < 2) {
      fprintf(stderr, "Missing argument : boolean to specify whether to predict or visualize filter\n");
      exit(0);
  }
  int predict = atoi(argv[1]);
  ...
  if(predict == 0) {
    ...
    if (argc < 5) {
      fprintf(stderr, "Missing argument : visualization layer\n");
        exit(0);
    }
    if (argc < 6) {
      fprintf(stderr, "Missing argument : filter to visualize\n");
      exit(0);
    }
    int net_idx = atoi(argv[3]);
    overfeat::init(argv[2], net_idx);
    
    int layer_vis = atoi(argv[4]);
    int filter_vis = atoi(argv[5]);
    overfeat::writeFilterToPPM(layer_vis, filter_vis);
  }
  else {
    ...
    int nTopClasses = atoi(argv[3]);
    int net_idx = atoi(argv[4]);
    ...
    while(readPPM(stdin, input_raw)) {
      ...
      if (nTopClasses > 0) { // print top classes
        ...
        // classification
        THTensor* output = overfeat::fprop(input, zero_layer, zero_perc);
        ...
        //overfeat::writeFilterToPPM(2, 50);
        ...
        */
        ...
        // extract features
        THTensor* output = overfeat::fprop(input_raw, zero_layer, zero_perc);
